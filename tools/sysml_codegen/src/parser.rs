...
use crate::ast::{
    ActionDef, Attribute, Connection, EnumDef, InputGroup, Package, PartDef, PartInstance, Port,
    Signal, SignalDirection, State, StateAction, StateMachine, Transition,
};

/// Parse a `SysML` 2 source file into a Package AST.
pub fn parse_sysml(content: &str, filename: &str) -> Package {
    let pkg_name = extract_package_name(content).unwrap_or_else(|| {
...
fn parse_action_defs(content: &str) -> Vec<ActionDef> {
    ...
    while i < content.len() {
        if let Some(pos) = content[i..].find("action def ") {
            ...
            let prefix_start = abs_pos.saturating_sub(10);
...
fn parse_action_refs(body: &str, kind_str: &str, action_defs: &[ActionDef]) -> Vec<StateAction> {
    ...
    for line in body.lines() {
        ...
        if let Some(rest) = trimmed.strip_prefix(&pattern) {
            ...
            // Look up the action def body and wcet
            let action_def = action_defs.iter().find(|ad| ad.name == name);
            let action_body = action_def.map(|ad| ad.body.clone());
            let wcet_us = action_def.and_then(|ad| ad.wcet_us);
...
fn parse_signals(body: &str) -> Vec<Signal> {
    ...
    for line in body.lines() {
        ...
        if let Some(colon) = rest.find(':') {
            ...
            let typ = if let Some(bracket) = type_part.find('[') {
                type_part[..bracket].trim().to_string()
            } else {
                type_part.to_string()
            };
            signals.push(Signal {
        ...
        if let Some(pos) = content[search_start..].find("part def ") {
            ...
            if let Some(brace_offset) = content[name_start..].find('{') {
                ...
                if let Some((body, end_pos)) = extract_balanced(content, body_start) {
                    let attributes = parse_attributes(&body);
                    let part_ports = parse_part_ports(&body, port_defs);
    ...
    for line in body.lines() {
        ...
        if let Some(rest) = trimmed.strip_prefix("port ") {
            ...
            if let Some(colon) = rest.find(':') {
                let name = rest[..colon].trim().to_string();
                let type_part = rest[colon + 1..].trim();
                let typ = if let Some(t) = type_part.strip_prefix('~') {
                    t.trim().to_string()
                } else {
                    type_part.to_string()
                };
...
fn resolve_state_actions_for(state: &mut State, action_defs: &[ActionDef]) {
        ...
        .chain(state.exit_actions.iter_mut())
    {
        if let Some(ad) = action_defs.iter().find(|ad| ad.name == action.name) {
            ...
            if action.wcet_us.is_none() {
                action.wcet_us = ad.wcet_us;
            }
        }
    }
}
...
fn parse_transitions(body: &str, action_defs: &[ActionDef]) -> Vec<Transition> {
    ...
    for stmt in &statements {
        let rest = stmt.strip_prefix("transition").unwrap_or("").trim();
        ...
        if let Some(then_pos) = rest.find(" then ") {
            ...
            let (before_actions, trans_actions) =
                extract_transition_actions(before_then, action_defs);

            let (from_state, condition, is_accept, accept_var) =
                parse_from_and_condition(before_actions);
...
fn extract_transition_actions<'a>(
    text: &'a str,
    action_defs: &[ActionDef],
) -> (&'a str, Vec<StateAction>) {
    let mut actions = Vec::new();
    ...
    if let Some(do_pos) = text.find(" do action : ") {
        ...
        let action_def = action_defs.iter().find(|ad| ad.name == action_name);
        let action_body = action_def.map(|ad| ad.body.clone());
        let wcet_us = action_def.and_then(|ad| ad.wcet_us);
...
/// Parse the part before `then` to extract `from_state`, condition, accept flag, and accept var.
/// Handles:
///   "FROM" -> (FROM, None, false, None)
///   "FROM if COND" -> (FROM, Some(COND), false, None)
///   "FROM accept EVENT" -> (FROM, Some(EVENT), true, Some(EVENT))
///   "FROM accept EVENT if COND" -> (FROM, Some(EVENT and COND), true, Some(EVENT))
fn parse_from_and_condition(before_then: &str) -> (String, Option<String>, bool, Option<String>) {
    ...
    if let Some(accept_pos) = before_then.find(" accept ") {
        ...
        if let Some(if_pos) = after_accept.find(" if ") {
            ...
            (from, Some(combined), true, Some(event.to_string()))
        } else {
            let event = after_accept.to_string();
            (from, Some(event.clone()), true, Some(event))
        }
        ...
        (from, Some(cond), false, None)
    } else {
        (before_then.to_string(), None, false, None)
    }
}
    ...
    while search_start < body.len() {
        ...
        if let Some(pos) = body[search_start..].find("state ") {
            ...
            if let Some(brace_offset) = body[name_start..].find('{') {
                let name = body[name_start..name_start + brace_offset]
                    .trim()
                    .to_string();
                let brace_start = name_start + brace_offset;
    ...
    for line in body.lines() {
        ...
        if let Some(rest) = trimmed.strip_prefix("connection ") {
            ...
            if let Some(to_pos) = rest.find(" to ") {
                ...
                let (src_instance, src_port) = if let Some(dot) = from_part_port.find('.') {
                    (
                ...
                let (dst_instance, dst_port) = if let Some(dot) = to_part_port.find('.') {
                    (
                ...
                connections.push(Connection {
                    from_part: src_instance,
                    from_port: src_port,
                    to_part: dst_instance,
                    to_port: dst_port,
                    capacity,
                });
            }
...
/// Extract `wcet N;` annotation from an action def body.
/// Returns `(wcet_value, cleaned_body)` — the body with the wcet line removed.
fn extract_wcet(body: &str) -> (Option<u64>, String) {
    let mut wcet = None;
    let mut clean_lines = Vec::new();

    for line in body.lines() {
        let trimmed = line.trim();
        if let Some(rest) = trimmed.strip_prefix("wcet ") {
            let rest = rest.trim_end_matches(';').trim();
            if let Ok(val) = rest.parse::<u64>() {
                wcet = Some(val);
                continue; // skip this line in the cleaned body
            }
        }
...
mod tests {
    ...
    fn parse_model(name: &str) -> crate::ast::Package {
        ...
        parse_sysml(&content, &path)
    }

    #[test]
    fn test_parse_button() {
        let pkg = parse_model("button.sysml");
        assert_eq!(pkg.name, "Button");
        assert_eq!(pkg.parts.len(), 1);

        let part = &pkg.parts[0];
        assert_eq!(part.name, "ButtonActor");

        let sm = part.state_machine.as_ref().expect("has state machine");
        ...
        assert_eq!(part.attributes.len(), 9);
        assert_eq!(part.ports.len(), 1);
        assert_eq!(part.ports[0].name, "button_out");

        let accept_transitions: Vec<&crate::ast::Transition> =
            sm.transitions.iter().filter(|t| t.is_accept).collect();
        assert_eq!(accept_transitions.len(), 1);
        assert_eq!(accept_transitions[0].from_state, "Idle");
        assert_eq!(accept_transitions[0].to_state, "Debouncing");
    }

    #[test]
    fn test_parse_control() {
        let pkg = parse_model("control.sysml");
        assert_eq!(pkg.name, "Control");

        let part = &pkg.parts[0];
        let sm = part.state_machine.as_ref().expect("has state machine");

        let conditional: Vec<&crate::ast::Transition> = sm
            .transitions
            .iter()
            .filter(|t| t.condition.is_some() && !t.is_accept)
            .collect();
        assert!(
            !conditional.is_empty(),
            "control should have conditional transitions"
        );
    }
    ...
    fn test_parse_maintenance() {
        ...
        assert_eq!(pkg.name, "Maintenance");

        let part = &pkg.parts[0];
        let sm = part.state_machine.as_ref().expect("has state machine");

        let from_checking: Vec<&crate::ast::Transition> = sm
            .transitions
            .iter()
            .filter(|t| t.from_state == "Checking")
            .collect();
        let cond_count = from_checking
            .iter()
            .filter(|t| t.condition.is_some())
            .count();
        let uncond_count = from_checking
            .iter()
            .filter(|t| t.condition.is_none())
            .count();
        assert_eq!(cond_count, 1, "one conditional from Checking");
        assert_eq!(uncond_count, 1, "one unconditional fallback from Checking");
    }

    #[test]
    fn test_parse_system() {
        let pkg = parse_model("system.sysml");
        assert_eq!(pkg.name, "System");

        let part = &pkg.parts[0];
        assert_eq!(part.name, "PicoSystem");

        assert_eq!(part.part_instances.len(), 3);
        let instance_names: Vec<&str> = part
            .part_instances
            .iter()
            .map(|i| i.name.as_str())
            .collect();
        assert!(instance_names.contains(&"button"));
        assert!(instance_names.contains(&"control"));
        assert!(instance_names.contains(&"maintenance"));

        assert_eq!(part.connections.len(), 2);
        let conn0 = &part.connections[0];
        assert_eq!(conn0.from_part, "button");
        assert_eq!(conn0.to_part, "control");
        assert_eq!(conn0.capacity, 4);
    }

    #[test]
    fn test_parse_action_defs_resolved() {
        let pkg = parse_model("button.sysml");
        assert!(pkg.action_defs.len() >= 3);

        let reset = pkg.action_defs.iter().find(|a| a.name == "reset_debounce");
        assert!(reset.is_some(), "missing reset_debounce action def");
        assert!(
            reset
                .expect("reset_debounce")
                .body
                .contains("debounce_counter := 0"),
            "missing body content"
        );
    }

    #[test]
    fn test_parse_state_defs_resolved() {
        let pkg = parse_model("button.sysml");
        let part = &pkg.parts[0];
        ...
        assert!(
            idle.entry_actions[0].body.is_some(),
            "entry action body should be resolved from action def"
        );
        ...
        assert!(debouncing.do_actions[0].body.is_some());
    }

    #[test]
    fn test_parse_port_defs() {
        let pkg = parse_model("button.sysml");
        assert_eq!(pkg.port_defs.len(), 1);
        let port = &pkg.port_defs[0];
        assert_eq!(port.name, "ButtonPort");
        assert_eq!(port.signals.len(), 2);

        let pressed = port.signals.iter().find(|s| s.name == "pressed_event");
