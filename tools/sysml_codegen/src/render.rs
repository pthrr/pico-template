...
//!
//! Mechanical `format!`-based rendering.

use crate::rust_ast::{
    Assignment, MethodBody, NewBody, RustEnum, RustImpl, RustItem, RustMethod, RustModule,
    RustStruct, RustTrait, RustTraitImpl, StepBody, TransitionCode,
};

/// Render a `RustModule` into a complete Rust source string.
pub fn render(module: &RustModule) -> String {
    let mut buf: Vec<String> = Vec::new();
    buf.push(format!("// package: {}\n", module.package_name));
...
fn render_trait_impl(buf: &mut Vec<String>, ti: &RustTraitImpl) {
    ...
    for method in &ti.methods {
        render_method_with_vis(buf, method, "    ", false);
    }
    buf.push("}".to_string());
    buf.push(String::new());
}
...
fn render_method(buf: &mut Vec<String>, method: &RustMethod, indent: &str) {
    render_method_with_vis(buf, method, indent, true);
}

/// Render a method with controllable visibility. Inside a trait `impl` block
/// methods must not carry a `pub` qualifier (the visibility is inherited from
/// the trait); inside inherent `impl` blocks `pub` is required.
fn render_method_with_vis(buf: &mut Vec<String>, method: &RustMethod, indent: &str, pub_vis: bool) {
    ...
    let is_const = method.name == "new" || method.name == "step";
    let const_kw = if is_const { "const " } else { "" };
    match &method.ret_type {
        Some(rt) => {
            buf.push(format!(
                "{indent}{vis}{const_kw}fn {}({}) -> {rt} {{",
                method.name, method.params
            ));
        }
        None => {
            buf.push(format!(
                "{indent}{vis}{const_kw}fn {}({}) {{",
                method.name, method.params
            ));
        }
    }
...
fn render_step_body(buf: &mut Vec<String>, body: &StepBody, indent: &str) {
    ...
    for arm in &body.arms {
        ...
        if !arm.transitions.is_empty() {
            ...
            let has_conditional = arm
                .transitions
                .iter()
                .any(|t| matches!(t, TransitionCode::Conditional { .. }));
                ...
                for trans in &arm.transitions {
                    match trans {
                        TransitionCode::Conditional {
                            ...
                            if is_first {
                                buf.push(format!("{indent}            if {condition} {{"));
                                is_first = false;
                            } else {
                                buf.push(format!("{indent}            }} else if {condition} {{"));
                            }
